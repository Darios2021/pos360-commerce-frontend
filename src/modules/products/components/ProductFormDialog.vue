  ...
  <v-dialog v-model="openLocal" max-width="1100" persistent>
    <v-card rounded="xl">
      <v-card-title class="d-flex align-center justify-space-between">
        ...
        <div class="d-flex align-center ga-2">
          ...
          <!-- ✅ eliminar solo admin y solo edit -->
          <v-btn
      ...
      <v-card-text>
        ...
        <v-window v-model="tab" class="mt-4">
          ...
          <v-window-item value="datos">
            <v-row dense>
              <v-col cols="12" md="6">
                <v-text-field v-model="model.name" label="Nombre" variant="outlined" />
              </v-col>
              <v-col cols="12" md="3">
                <v-text-field v-model="model.sku" label="SKU" variant="outlined" />
              </v-col>
              <v-col cols="12" md="3">
          ...
          <v-window-item value="precios">
            <v-row dense>
              <v-col cols="12" md="4">
                <v-text-field
                  v-model.number="model.price_list"
                  label="Precio Lista"
                  variant="outlined"
                  type="number"
                />
          ...
          <v-window-item value="imagenes">
            <v-card rounded="xl" variant="tonal" class="pa-4">
              ...
              <v-file-input
                ...
                multiple
                accept="image/*"
                :disabled="mode !== 'edit' || !model?.id"
                :hint="mode !== 'edit' ? 'Guardá el producto primero para subir imágenes.' : ''"
                persistent-hint
              />
              ...
              <div class="d-flex justify-end mt-3">
                <v-btn
                  ...
                  :disabled="mode !== 'edit' || !model?.id || !newFiles.length"
                  :loading="uploading"
                >
                  Subir seleccionadas
                </v-btn>
              ...
              <div class="text-caption text-medium-emphasis mt-2">
                Al guardar: se guardan URLs en <b>product_images</b>. (subida manual con botón de arriba)
              </div>
            </v-card>
          </v-window-item>
        </v-window>
      ...
      <v-card-actions class="justify-end">
        <v-btn variant="tonal" @click="close" :disabled="products.loading">Cancelar</v-btn>
        <v-btn color="primary" variant="flat" @click="save" :loading="products.loading">
          Guardar
        </v-btn>
      </v-card-actions>
    </v-card>
    ...
    <v-dialog v-model="deleteImgOpen" max-width="520">
      <v-card rounded="xl">
        ...
        <v-card-text>
          ¿Eliminar la imagen ID <b>#{{ deleteImg?.id }}</b>?
          <div class="text-caption text-medium-emphasis mt-2">Se borra de la BD (y de MinIO si está activado).</div>
        </v-card-text>
        <v-card-actions class="justify-end">
...
watch(
  ...
  async (v) => {
    ...
    if (v) {
      ...
      // ✅ si edit: cargar imágenes existentes
...
async function save() {
  ...
  if (props.mode === "edit" && model.value.id) {
    saved = await products.update(model.value.id, payload);
  } else {
    saved = await products.create(payload);
  }

  if (saved?.id) {
    model.value.id = saved.id;

    // refrescar imágenes si edit
    if (props.mode === "edit") {
