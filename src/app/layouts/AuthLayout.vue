...
// ✅ DARK MODE (persist)
// --------------------
const THEME_KEY = "pos360_theme"; // "light" | "dark"

const isDark = computed(() => theme.global.current.value.dark);

function applyTheme(name) {
  // name debe existir en createVuetify({ theme: { themes: { light, dark }}})
...
onMounted(() => {
  // intenta respetar preferencia guardada
  let saved = "";
  try {
...
function normalizeUrl(u) {
  ...
  // si viene /uploads/... o similar -> antepone base
  const base =
    String(import.meta.env.VITE_ASSETS_BASE_URL || import.meta.env.VITE_API_BASE_URL || "")
      .replace(/\/$/, "");

  if (!base) {
    // si no hay base, devolvemos tal cual para que al menos intente relativo
    return s;
  }
...
const avatarSrc = computed(() => normalizeUrl(userAvatarRaw.value));
